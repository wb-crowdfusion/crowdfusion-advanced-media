...
MediaLibraryTagWidget.prototype._handlePostInitialize = function() {
  ...
  $('textarea[data-provide="markdown"]').each(function(i,e) {
    var editor = $(e);
    var overlay = me._createDropOverlay.apply(me, ['drop to add tag','release to add tag', e.id]);
    overlay.droppable({
      accept: '.media-draggable',
      ...
      drop: function(event, ui) {
        ...
        if (url != null) {
          ...
          var mdeTextArea = editor[0];
          var startPos = mdeTextArea.selectionStart;
          var endPos = mdeTextArea.selectionEnd;

          var textAreaValue=editor.val().substring(0, startPos) + value + editor.val().substring(endPos, editor.val().length);
          editor.val(textAreaValue);
          editor.focus();
          mdeTextArea.selectionStart = startPos + value.length;
          mdeTextArea.selectionEnd = startPos + value.length;
        }
      },
      tolerance: 'pointer'
    });
